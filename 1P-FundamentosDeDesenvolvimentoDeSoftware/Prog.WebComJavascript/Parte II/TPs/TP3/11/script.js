...
const agrupadoPorPais = figurasHistoricas.reduce((acc, figura) => {
  const pais = figura.paisOrigem;
  if (!acc[pais]) {
    acc[pais] = [];
  }
  acc[pais].push(`${figura.nome} ${figura.sobrenome}`);
  return acc;
}, {});

console.log(agrupadoPorPais);
